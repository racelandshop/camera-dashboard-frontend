...
class HaSidebar extends LitElement {
  ...
  private _racelandFilter(beforeSpacer: any[], afterSpacer: any[]) {
    if (this.hass.user?.is_owner) {
      return [beforeSpacer, afterSpacer];
    }

    const pannelsToFilterByTitle = [
      "energy",
      "map",
      "logbook",
      "history",
    ...
    if (this.hass.user?.is_admin) {
      afterSpacerFiltered = afterSpacer.filter(
        (sideBarDiv) => sideBarDiv.component_name !== "developer-tools"
      );
    } else {
      afterSpacerFiltered = afterSpacer;
    }

    beforeSpacerFiltered = beforeSpacer.filter(
      (sideBarDiv) =>
        !pannelsToFilterByTitle.includes(
          sideBarDiv.title ? sideBarDiv.title : "null"
        )
    );
  ...
  private _renderAllPanels() {
    ...
    const [beforeSpacer, afterSpacer] = this._racelandFilter(
      beforeSpacerPreFilter,
      afterSpacerPreFilter
    );
    // prettier-ignore
    return html`
      <paper-listbox
