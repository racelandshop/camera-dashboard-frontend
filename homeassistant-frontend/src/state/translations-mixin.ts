  ...
  class extends superClass {
    ...
    protected hassConnected() {
      ...
      getUserLocale(this.hass!).then((locale) => {
        ...
        if (locale?.theme && this.hass!.locale.theme !== locale.theme) {
          // We just got theme from backend, no need to save back
          this._selectTheme(locale.theme, false);
        }
      });
    ...
    // TODO change type
    private _selectTheme(theme: object, saveToBackend: boolean) {
      if (!this.hass) {
      ...
      this._updateHass({
        locale: {
          ...this.hass!.locale,
          theme: { ...this.hass!.locale.theme, ...theme },
        },
        themes: { ...this.hass!.themes, theme: theme.theme },
        selectedTheme: theme,
      });
      ...
      if (theme.dark === true) {
        darkMode = true;
      } else if (theme.dark === false) {
        darkMode = false;
      } else {
        darkMode = undefined;
      }
      this._updateHass({
        themes: { ...this.hass!.themes, darkMode: darkMode },
      });
